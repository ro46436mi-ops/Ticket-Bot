...
class TicketCommands(commands.Cog):
    ...
    async def create_ticket(self, interaction: discord.Interaction, category_value):
        ...
        try:
            # Check blacklist - FIXED: Check if collection exists
            ...
            # Check existing tickets - FIXED: Check if collection exists
            ...
            # Save to database - FIXED: Check if collection exists
            if tickets_col is not None:
            ...
            class TicketButtons(discord.ui.View):
                ...
                async def close_button(self, interaction: discord.Interaction, button: discord.ui.Button):
                    # Check permissions - FIXED: Check if collection exists
                    ticket_data = None
                    if tickets_col is not None:
    ...
    async def close(self, interaction: discord.Interaction):
        ...
        # Get ticket data - FIXED: Check if collection exists
        ticket_data = None
        if tickets_col is not None:
            ticket_data = tickets_col.find_one({"channel_id": str(interaction.channel.id)})
    ...
    @app_commands.command(name="blacklist", description="⛔ Blacklist user from creating tickets")
    @app_commands.default_permissions(administrator=True)
...
# Add cog to bot
async def setup_cog():
    await bot.add_cog(TicketCommands(bot), guild=discord.Object(id=Config.GUILD_ID))
...
async def on_message(message):
    ...
    # Save to database if in ticket channel - FIXED: Check if collection exists properly
    if tickets_col is not None and message.channel.name.startswith('ticket-'):
        tickets_col.update_one(
